/* =========================================================================
   Montinho Training Strategy — Knowledge Base (Módulo 5)
   Biblioteca interna de conhecimento. Usada durante a entrevista para
   justificar decisões automaticamente, sempre em linguagem simples para o
   aluno. Nunca substitui o treinador — apenas fundamenta e enriquece.

   Base: consenso entre evidências atuais (meta-análises, revisões, ensaios) e
   experiência prática consolidada. Nunca copia nem defende opiniões isoladas.
   Cada entrada carrega um nível de confiança.
   ========================================================================= */
window.MTS = window.MTS || {};

MTS.Knowledge = (function () {
  var CONF_ORDER = { 'Muito alto': 4, 'Alto': 3, 'Moderado': 2, 'Baixo': 1 };

  /* -----------------------------------------------------------------------
     ENTRADAS. Cada uma responde: o que é, por que usar, benefícios, riscos,
     quando usar, quando evitar e como explicar ao aluno (adaptável ao perfil).
     ----------------------------------------------------------------------- */
  var ENTRIES = [
    /* ---------- Conceitos-base ---------- */
  ...
  /* -----------------------------------------------------------------------
     ENGINE
     ----------------------------------------------------------------------- */
  function textOf(v) { return Array.isArray(v) ? v.join(' ') : String(v == null ? '' : v); }

  function match(text) {
    var t = textOf(text);
    if (!t.trim()) return [];
    return ENTRIES.filter(function (e) { return e.match.test(t); });
  }

  function byId(id) { return ENTRIES.filter(function (e) { return e.id === id; })[0] || null; }
  function all() { return ENTRIES.slice(); }

  /* Perfil do aluno para adaptar a explicação. */
  function profileKeys(state) {
    var a = (state && state.anamnese) || {}, keys = [];
    var idade = parseInt(a.idade, 10);
    var obj = String(a.objetivo || '').toLowerCase();
    var exp = String(a.experiencia || '').toLowerCase();
    ...
    if (String(a.sexo || '').toLowerCase() === 'feminino') keys.push('feminino');
    return keys;
  }
  var PROFILE_LABEL = { idoso: 'idoso', atleta: 'atleta', emagrecimento: 'emagrecimento', reabilitacao: 'reabilitação', iniciante: 'iniciante', feminino: 'treino feminino' };

  /* Explicação para o aluno, adaptada ao perfil quando houver variação. */
  function explain(entry, state) {
    var keys = profileKeys(state);
    for (var i = 0; i < keys.length; i++) {
      if (entry.adapt && entry.adapt[keys[i]]) return { text: entry.adapt[keys[i]], profile: PROFILE_LABEL[keys[i]] };
    }
    return { text: entry.student, profile: null };
  }

  /* Nota sobre o nível de confiança (evidência). */
  function confidenceNote(entry) {
  ...
  function confidenceClass(entry) {
  ...
  function forTopic(state, topicId) {
    var topic = (MTS.TOPICS || []).filter(function (t) { return t.id === topicId; })[0];
    if (!topic) return [];
    var ans = state.answers || {};
    var texts = MTS.Interview.questionsForTopic(topic, state).map(function (q) { return textOf(ans[q.id]); });
    // também considera dores/lesões da anamnese nas condições
    if (topicId === 'exercicios' || topicId === 'mobilidade')
      texts.push(textOf((state.anamnese || {}).dores), textOf((state.anamnese || {}).lesoes));
    var found = {}, out = [];
    texts.forEach(function (t) {
      match(t).forEach(function (e) { if (!found[e.id]) { found[e.id] = 1; out.push(e); } });
    });
    return out;
  }

  return {
    all: all, byId: byId, match: match, forTopic: forTopic,
    explain: explain, profileKeys: profileKeys,
    confidenceNote: confidenceNote, confidenceClass: confidenceClass,
